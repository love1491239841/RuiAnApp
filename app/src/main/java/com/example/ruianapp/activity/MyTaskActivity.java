...
import com.example.ruianapp.adapter.MyTaskAdapter;
...
public class MyTaskActivity extends AppCompatActivity {
    ...
    private void initView(){
        recyclerView= (RecyclerView) findViewById(R.id.mytask_recycler);
    ...
    private void initData(final String userId){
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    UtlisOkhttp.sendIdOkHttpRequest(userId, Constants.MyTask_URL,new okhttp3.Callback(){
                        @Override
    ...
    private void showResponse(final String response) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                try{
                    System.out.println("buhavcavshcbj"+response);
                    adapter = new MyTaskAdapter(JsonGet.myTasks(response),MyTaskActivity.this);
